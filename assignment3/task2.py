...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
       for (X_batch, Y_batch) in dataloader:
            ...
            accuracy.append((output_probs.argmax(axis=1) == Y_batch).sum().item() / Y_batch.shape[0])
...
class ExampleModel(nn.Module):
   ...
   def __init__(self,
       ...
       """
       super().__init__()
       num_filters = [32, 64, 128]  # Set number of filters in conv layers
       ...
       self.num_output_features = num_filters[2] * 4 * 4
...
def calculate_final_loss(trainer: Trainer):
    validation_loss, validation_acc = compute_loss_and_accuracy(
    ...
    print(
        f"Training_loss: {training_loss:.3f}",
        f"Training_acc: {training_acc:.3f}",
        f"",
        f"Validation_loss: {validation_loss:.3f}",
        f"Validation_accuracy: {validation_acc:.3f}",
        f"",
...
if __name__ == "__main__":
    # Task 2 hyperparameters and calls
    epochs = 10
    batch_size = 64
    learning_rate = 5e-2
    early_stop_count = 4
